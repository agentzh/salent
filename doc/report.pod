...
本文档描述了于 2005 年 9 月发布的 Salent 0.08
...
L<ModelSim::List>, L<Template::Plugin::POSIX>,
L<Template::Plugin::Perl>, L<Template::Ast>,
和 L<Win32::Wildcard>。另外，我的流程图生成器 Flowviz，Makefile
构造流程生成器 L<Makefile::GraphViz> 及相关联的 L<Makefile::Parser>
...
ALU 不是做指令译码器，单纯的 ALU 操作没有我们想象的那么多么。
...
另一问题来自 Intel 的指令码表本身。虽然"IA-32 Intel  ArchitectureSoftware
...
"IA-32 Intel  ArchitectureSoftware Developer’s Manual" 第二卷中的 
Appendix B.
...
我在邀请我们专业的何杉同学为我们的 Salent IDU 提供 Java 实现的模板的时候曾经在 QQ 中这样向他解释如何将我们已有的 Perl 实现的模板改写为 Java 实现的模板：
...
Tree. The best way to comprehend ASTs is simply reguard it as a complex data structure. Hmm...maybe something like a tree, an array, a hash, or so so...
...
Appendix B。我已用一些第三方的反汇编器对我的 Perl 脚本生成的 x86 机器指令
进行了测试。
